...
function createSnippet(option: TSnippetOption) {
  ...
  // 添加连线
  const edges: Edge.Metadata[] = snippetProps.edges?.map((edgeConfig: any) => {
    const sourceCell = idMap[edgeConfig.source.cell];
    const targetCell = idMap[edgeConfig.target.cell];
    const [sourceCellPort] = sourceCell.getPortsByGroup(edgeConfig.source.port);
    const [targetCellPort] = targetCell.getPortsByGroup(edgeConfig.target.port);
    return {
      attrs: {
        line: {
